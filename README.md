...
| `CORS_ALLOWED_ORIGINS` | Orígenes permitidos por el middleware CORS | `http://localhost:5173,http://localhost:8000` |
...
| `AiController.php` | **Chat IA "Profesor Euler"** (Groq): proxy seguro que protege la API key. Límite diario de 50 requests por token (`daily_requests`), prompt de sistema que restringe el asistente a matemáticas y prohíbe resolver tareas, y **respuesta en streaming SSE**. |
